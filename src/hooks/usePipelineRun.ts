...
export function usePipelineRun({ onNodesChange, onEdgesChange }: UsePipelineRunOptions) {
  const [isRunning, setIsRunning] = useState(false);
  const [streamingLog, setStreamingLog] = useState<string | null>(null);

  const runPlan = useCallback(async (pipelineId: string, params: Record<string, string> = {}) => {
    setIsRunning(true);
    try {
  ...
  const runStreaming = useCallback(async (pipelineId: string, params: Record<string, string> = {}) => {
    setIsRunning(true);
    ...
    try {
      ...
      for await (const event of watchRun({ runId })) {
        ...
        if (event.eventType === "EVENT_TYPE_COMPLETE" && event.clientView?.graph) {
          const { nodes, edges } = transformApiGraphToReactFlow(event.clientView.graph);
          onNodesChange(nodes);
          onEdgesChange(edges);
        }
      ...
      setStreamingLog("Error: " + (err instanceof Error ? err.message : String(err)));
    } finally {
      setIsRunning(false);
    }
  }, [onNodesChange, onEdgesChange]);
